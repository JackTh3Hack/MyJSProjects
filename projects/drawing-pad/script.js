...
function draw() {
	...
	if (mouseIsPressed && pencil.checked) {
		// Store the location of the mouse & the color and weight of the stroke
		const point = { x: mouseX, y: mouseY, color: colorInput.value, weight: weight.value };
		// Adding the point to the currentPath array
		currentPath.push(point);
	}
	else if (mouseIsPressed && eraser.checked) {
		const point = { x: mouseX, y: mouseY, color: "#fff", weight: weight.value };
		currentPath.push(point);
	}
